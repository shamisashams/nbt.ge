        ...
        <div class="col-lg-6 col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div>
                        <h6 class="card-title mb-1">@lang('admin.projects')</h6>
                    </div>
                    <?php
                    $ids = $product->projects->pluck("id")->toArray();
                    ?>
                    <div class="mb-4">
                        @foreach($projects as $project)
                            <div class="form-group">
                                <label class="ckbox">
                                    <input type="checkbox" name="project[]" data-checkboxes="mygroup" class="custom-control-input"  id="{{$project->id}}" value="{{$project->id}}" {{in_array($project->id,$ids) ? 'checked' : ''}}>
                                    <span style="margin-left: 5px">{{$project->title}}</span>
                                ...
                                @if($item->type == 'select')
                                    <select class="form-control" name="attribute[{{$item->id}}]">
                                        ...
                                        @foreach($item->options as $option)
                                            ...
                                            ?>
                                            <option value="{{$option->id}}"{{$selected}}>{{$option->code}} {{$option->label}} {{$option->value}} {{$option->color}}</option>
                                        @endforeach
                                    </select>
                                @else
